    ...
    public function index(Request $request)
    {
        ...
        if (request()->ajax()) {
            if (!empty($request->from_date)) {
                $data = Task::with('employee')->select('id', 'title', 'start_date', 'due_date', 'duration', 'type', 'status', 'employee_id')->whereBetween('due_date', array($request->from_date, $request->to_date))
                    ->get();
            } else {
            ...
            return datatables()->of($data)
                ...
                ->editColumn('type', function ($data) {
                    if ($data->type == 1) return '<span class="badge bg-light text-dark"> <i class="fa fa-phone" aria-hidden="true"></i> Call</span>';
                    if ($data->type == 2) return '<span class="badge bg-light text-dark"><i class="fa fa-clock-o"></i> Deadline</span>';
                    if ($data->type == 3) return '<span class="badge bg-light text-dark"><i class="fa fa-envelope" aria-hidden="true"></i>Email</span>';
                    if ($data->type == 4) return '<span class="badge bg-light text-dark"><i class="fa fa-users"></i>Meeting</span>';
                    return '';
                })
                ->addColumn('status', function ($data) {
                    if ($data->status == 1) return '<span class="badge bg-warning">Ongoing</span>';
                ...
                ->addColumn('action', function ($data) {
                    ...
                    ' . csrf_field() . '
                    ' . method_field("DELETE") . '
                    <button type="submit" class="btn btn-sm btn-danger show_confirm" id=""> <i class = "fa fa-trash"></i> Delete</button>
                    </form>';
                    return $edit. $delete;
                })
